// created by filip.sound@gmail.com, May 2013
...
int sprite(Frame frame, vec2 p)
{
	int d = 0;
	p = floor(p);
	if (p.x >= 0.0 && p.x < 6.0 && p.y >= 0.0 && p.y < 10.0)
	{
		int s = int(p.y / 2.0);
...
void main(void)
{
	...
	// swirl
	float h = smoothstep(2.0, 60.0, abs(gl_FragCoord.x-mouse));
	vec2 q = p;
	...
	p.y += 9.0-f;
	if (iMouse.z > 0.0) p.x *= 1.0 - 2.0 * step(gl_FragCoord.x, mouse);
	else p.x *= 1.0 - 2.0 * step(mouse, gl_FragCoord.x);
	p.x = mod(p.x + floor(time*4.0), 12.0);
	
	f = length(gl_FragCoord.xy-vec2(mouse, max(iResolution.y *0.5 - 30.0, gl_FragCoord.y)));
	p.y += floor(60.0*(smoothstep(40.0, 0.0, floor(f)))); // elevation
	i = sprite(frame(frames, int(mod(floor(time*4.0+1.0), 8.0))), p);
	h = smoothstep(10.0, 120.0, abs(gl_FragCoord.x-mouse));
	...
	// invaders
	p = 0.25*(gl_FragCoord.xy - vec2(mouse, iResolution.y));
	...
	h = mod(hash(t*floor(p.y/20.0)),33554430.0);
	f = invader(floor(q), h, 0.0) * smoothstep(iResolution.y*0.6, iResolution.y, gl_FragCoord.y);
